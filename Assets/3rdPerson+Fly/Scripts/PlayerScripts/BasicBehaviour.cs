...
public class BasicBehaviour : MonoBehaviour
{
	...
	private void Start()
	{
		...
		nickText.text = _pv.Owner.NickName;
		if (_pv.Owner.NickName == "Admin")
		{
			for (int i = 0; i < kalpler.Length; i++)
			{
				kalpler[i].gameObject.SetActive(true);
			}
		}
		if (_cameraWork != null)
		{
	...
	void Update()
	{
		yazilar = GameObject.FindObjectsOfType<Canvas>();
		for (int i = 0; i < yazilar.Length; i++)
		{
			yazilar[i].transform.rotation = GameObject.FindGameObjectWithTag("MainCamera").transform.rotation;
		}
		if (_pv.IsMine)
		{
